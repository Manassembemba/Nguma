...
-- Migration: Corrige l'insertion de notification dans approve_withdrawal pour correspondre au schéma
-- Date: 2025-12-04
-- Description: Les tentatives précédentes ont échoué à cause d'une incompatibilité entre la fonction
-- et la structure réelle de la table 'notifications'. Cette version finale retire les colonnes
-- 'type' et 'priority' de l'insertion pour s'aligner sur les autres fonctions du projet.

-- Supprimer la version existante et incorrecte de la fonction
DROP FUNCTION IF EXISTS public.approve_withdrawal(UUID, TEXT);

-- Recréer la fonction avec l'instruction INSERT finale et correcte
...
BEGIN
    ...
    -- CORRECTION FINALE ET DÉFINITIVE: Aligner sur les autres fonctions
