...
-- Migration: Corrige le nom de la colonne 'link' en 'link_to' dans la fonction approve_withdrawal
-- Date: 2025-12-04
-- Description: La migration précédente a corrigé le problème de la colonne 'title', mais a laissé une erreur sur la colonne 'link'.
-- Le nom correct de la colonne est 'link_to'. Cette migration corrige la fonction une bonne fois pour toutes.

-- Supprimer la version existante et incorrecte de la fonction
DROP FUNCTION IF EXISTS public.approve_withdrawal(UUID, TEXT);

-- Recréer la fonction avec le nom de colonne 'link_to' corrigé
...
BEGIN
    ...
    -- CORRECTION FINALE: Utiliser 'link_to' au lieu de 'link'
    INSERT INTO public.notifications (user_id, message, type, priority, link_to)
    VALUES (
