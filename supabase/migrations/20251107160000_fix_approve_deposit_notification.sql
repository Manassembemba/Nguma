-- Final fix for the approve_deposit function, removing the non-existent 'title' column from the notification insert.
...
BEGIN
    ...
    -- Create a notification for the user (with title removed)
    INSERT INTO public.notifications (user_id, message, reference_id)
    VALUES (transaction_record.user_id, 'Votre dépôt de ' || transaction_record.amount || ' a été approuvé.', transaction_id_to_approve);
