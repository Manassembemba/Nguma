-- ==============================================================================================
-- SCRIPT SQL DÉFINITIF pour la fonction get_investor_list_details
-- Copiez et exécutez ce script complet dans votre éditeur SQL Supabase.
-- ==============================================================================================

-- Étape 1: Suppression agressive de toutes les signatures de fonction potentiellement conflictuelles.
...
-- Étape 2: Création de la version "finale" et correcte de la fonction.
...
BEGIN
  ...
  WHERE
    ...
    AND (p_status_filter IS NULL OR investor_status = p_status_filter)
  ...
  FROM (
    SELECT
      ...
      NULL as banned_until, -- La colonne n'existe pas dans profiles, on la laisse à NULL pour correspondre à la structure attendue par le frontend.
    ...
    WHERE
      ...
      AND (p_status_filter IS NULL OR iws.investor_status = p_status_filter)
